          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {allOrders && allOrders?.length > 0 ? (
              allOrders?.map((orderItem) => (
                <tr key={orderItem._id} className="hover:bg-gray-100 transition-colors">
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{orderItem._id}</td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">₹{orderItem.totalAmount}</td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    {/* Order status dropdown */}
                    <select
                      className="border p-1 rounded"
                      value={newStatus[orderItem._id] || ""}
                      onChange={(e) => setNewStatus({ ...newStatus, [orderItem._id]: e.target.value })}
                    >
              ...
              <tr>
                <td colSpan="5" className="text-center py-4 text-sm text-gray-500">No orders found.</td>
              </tr>
            )}
          </tbody>
        </table>
        ...
        {selectedOrder && (
          <div className="fixed inset-0   flex items-center justify-center bg-black bg-opacity-50 ">
            <div className="bg-white p-6 rounded-lg shadow-lg max-w-lg bg-[#F2F2F2]">
              ...
              <p>
                <strong>Address:</strong> {selectedOrder.addressInfo.address}, {selectedOrder.addressInfo.city},{" "}
                {selectedOrder.addressInfo.pincode}
              </p>
              <p><strong>Phone:</strong> {selectedOrder.addressInfo.phone}</p>
              <p><strong>Notes:</strong> {selectedOrder.addressInfo.notes?selectedOrder.addressInfo.notes:"None"}</p>

              <h4 className="mt-4 text-lg font-semibold">Products:</h4>
              <ul className="list-disc pl-5">
                {selectedOrder.cartItems?.map((product) => (
                  <li key={product._id}>
                    <strong>{product.productName}</strong> - {product.category}, Price: ₹{product.price}, Quantity: {product.quantity}
                  </li>
                ))}
              </ul>
              <button className="mt-4 bg-red-600 text-white px-4 py-2 rounded" onClick={() => setSelectedOrder(null)}>
                Close
              </button>
            </div>
          </div>
